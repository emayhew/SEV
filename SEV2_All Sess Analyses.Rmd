...
## Plot training sweetness data with test data
df2 <- df %>%
...
passfail <- left_join(SSE.df, df2, by = "SubjectCode") %>%
    transmute(SubjectCode,
              Session,
              Sample = `Sample.y`,
              SampleCode = `SampleCode.y`,
              Rating = `Rating.y`,
              StrongestSweet) %>%
    filter(!(SampleCode %in% c("A", "B", "C", "D", "E", "F", "G", "H", "I", "K", "L", "M", "O", "P","Q", "R", "T", "U", "V", "W", "X", "Y"))) %>%
    mutate(PassFail = case_when(StrongestSweet < Rating ~ "Fail", TRUE ~ "Pass")) 

## Make data table showing total number of times each subject rated a sample as being sweeter than their strongest sweetness experienced
passfail.total <- passfail %>%
  group_by(SubjectCode, PassFail) %>%
  count() %>%
  pivot_wider(names_from = PassFail, values_from = n) %>%
