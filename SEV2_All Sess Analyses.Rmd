...
## Sweetness and flavor rating means and SE for mixture samples only
df %>%
  ...
  ggplot(aes(Sample, Mean, color = Sensation, shape = Sensation)) +
    geom_point() +
    geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), width = 0.5) +
    geom_hline(yintercept = 12.295731, color = "tomato1") +
    geom_hline(yintercept =  18.571098, color = "deepskyblue3") +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    ggtitle("Mean Sensation Ratings of Mixture Samples")

ggsave("plots/SEV2/SEV2_MeanSE_FlavSweet_MixtureSamplesOnly.png", height = 5, width = 9)
...
## Making a new dataframe that includes both the test and training data
df2 <- df %>%
...
ctrl.means <- df %>%
 ...
 ## Plot sweetness means of normalized data
...
norm.df %>%
  ...
  ggplot(aes(reorder(Sample, Norm_Mean), Norm_Mean)) +
   ...
   geom_hline(yintercept =  0, color = "deepskyblue3") +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    ggtitle("Normalized Mean Sweetness Ratings of Mixtures")

ggsave("plots/SEV2/SEV2_NormMeanSE_Sweetness_MixturesOnly.png", height = 5, width = 9)



 ## Plot sweetness means of normalized data per subject, line graph
...
ggsave("plots/SEV2/SEV2_NormMean_Sweetness_Line.png", height = 5, width = 9)
 ...
 ## Plot sweetness means of normalized data per subject, stacked bar graph
